...
pub fn draw(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let info_text = Text::from(vec![
        ...
        Line::from(vec![
            ...
            Span::raw(app.room.description.as_str()),
        ]),
        Line::from(vec![
            Span::styled("Exits: ", Style::default().add_modifier(Modifier::BOLD)),
            Span::raw(app.room.exits.join(", ")),
        ]),
    ]);
    let info = Paragraph::new(info_text)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .title("ROOM INFO & EXITS"),
        )
        .wrap(Wrap { trim: true });
    let entity_chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
    ...
    let npc_lines = app
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let npcs = Paragraph::new(npc_lines)
